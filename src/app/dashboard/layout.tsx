...
export default async function DashboardLayout({
  ...
  let { data: profile } = await supabase
    .from('profiles')
    .select('role, nama_lengkap, nama_kampus, instansi')
    .eq('id', user.id)
    .single()
  ...
  if (!finalProfile) {
    // Attempt to insert a default profile for the user
    ...
    if (insertError) {
      console.error('Failed to create user profile in dashboard layout:', insertError)
      return (
  ...
  return (
    <DashboardShell
      ...
      namaLengkap={finalProfile.nama_lengkap || 'Pengguna'}
      namaKampus={finalProfile.role === 'admin' ? (finalProfile.instansi || 'Kesbangpol') : (finalProfile.nama_kampus || '')}
    >
      {children}
    </DashboardShell>
  )
